...
export class AccountsPage extends BasePage {
  ...
  async createEmergencyFundAccount(openingBalance: number): Promise<void> {
    await this.openCreateModal();
    await this.accountTypeButton("EMERGENCY_FUND").click();
    await this.openingBalanceInput.fill(String(openingBalance));
    await this.submitAndWait();
  }
   ...
   * archives first if the account isn't already archived. */
  async deleteAccount(accountName: string, opts?: { alreadyArchived?: boolean; alsoDeleteTransactions?: boolean }): Promise<void> {
    ...
    if (opts?.alsoDeleteTransactions) {
      await this.page.getByRole("dialog").getByText("Also delete its expense and income entries").click();
    }
    await Promise.all([
