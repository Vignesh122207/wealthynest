-- Budgets are reusable templates: period_month/period_year are always 0 in storage — the actual
-- month/year is supplied by the caller at read time, and spend is computed live against that
-- period. This is why there's no "copy budget to next month" feature; every budget already
-- applies to every period by construction.
