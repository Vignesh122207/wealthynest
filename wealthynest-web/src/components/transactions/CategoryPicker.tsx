...
import {ChevronDown, type LucideIcon, Tag} from "lucide-react";
...
export function CategoryPicker({ label = "Category", placeholder = "Select category", categories, value, onChange, error, iconFor, manageHref, compact = false, testId = "category-picker" }: {
   ...
   * more than one CategoryPicker renders in the same form. */
  testId?: string;
}) {
      ...
      <DropdownPanel anchorRef={triggerRef} open={open} onClose={() => setOpen(false)}>
        <div data-testid={`${testId}-panel`} className="flex-1 min-h-0 overflow-y-auto">
          {categories.map(c => {
            ...
            return (
              <button key={c.value} type="button" data-testid={`${testId}-option-${c.value}`} onClick={() => { onChange(c.value); setOpen(false); }}
