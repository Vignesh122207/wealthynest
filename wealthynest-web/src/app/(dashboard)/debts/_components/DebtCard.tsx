...
function StatusBadge({ status }: { status: DebtRecord["status"] }) {
  if (status === "SETTLED")
    return <span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="w-3 h-3" />Settled</span>;
  if (status === "PARTIAL")
    return <span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400"><Clock className="w-3 h-3" />Partial</span>;
  return <span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400"><AlertCircle className="w-3 h-3" />Active</span>;
}
...
export function DebtCard({ debt, onEdit, onPayment }: {
  ...
  return (
    <div data-testid="debt-card" className={cn(
      "bg-card border rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-md hover:shadow-black/5 dark:hover:shadow-black/20",
      isSettled ? "border-border/50 opacity-60" : isLent ? "border-emerald-500/20 hover:border-emerald-500/35" : "border-red-500/20 hover:border-red-500/35"
    )}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2 justify-between">
              <div className="min-w-0">
                <p className="text-[15px] font-bold text-foreground leading-tight truncate">{debt.contactName}</p>
                ...
                {debt.dueDate && new Date(debt.dueDate) < new Date() && debt.status !== "SETTLED" && (
                  <span className="inline-block text-[11px] font-medium px-2 py-0.5 rounded-full bg-red-500/10 text-red-600 dark:text-red-400 mt-1.5">Overdue</span>
                )}
              </div>
              <div className="flex flex-col items-end gap-1.5 shrink-0">
                <p className={cn("text-lg sm:text-xl font-extrabold tabular-nums tracking-tight leading-none", isLent ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400")}>
                  {fmt(debt.amount)}
                </p>
            ...
            {debt.description && (
              <p className="text-xs text-muted-foreground/80 mt-1.5 truncate">{debt.description}</p>
            )}
          </div>
        </div>
      </button>
      ...
      <div className="px-4 sm:px-5 pb-4 sm:pb-5 -mt-1 flex items-center gap-2.5">
        {!isSettled && (
          <button data-testid="debt-card-pay-button" onClick={onPayment}
            className={cn(
              "flex items-center gap-1.5 h-9 px-3.5 rounded-full text-xs font-semibold text-white shrink-0 transition-all duration-200 hover:-translate-y-0.5",
              isLent
                ? "bg-emerald-600 hover:bg-emerald-500 shadow-sm shadow-emerald-600/20 hover:shadow-lg hover:shadow-emerald-600/30"
                : "bg-red-600 hover:bg-red-500 shadow-sm shadow-red-600/20 hover:shadow-lg hover:shadow-red-600/30"
            )}>
        ...
        {debt.payments.length > 0 ? (
          <button onClick={() => setExpanded(v => !v)}
            className="flex-1 min-w-0 flex items-center justify-between gap-2 text-muted-foreground hover:text-foreground transition-colors duration-200 bg-muted/40 hover:bg-muted/60 rounded-2xl px-3.5 py-2">
      ...
      {expanded && debt.payments.length > 0 && (
        <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-2 border-t border-border/50 pt-3.5 animate-fade-in">
          {debt.payments.map(p => (
            <div key={p.id} className="flex items-center justify-between gap-2 text-xs">
              ...
              <span className="font-semibold text-foreground tabular-nums shrink-0">{fmt(p.amount)}</span>
            </div>
          ))}
        </div>
      )}
