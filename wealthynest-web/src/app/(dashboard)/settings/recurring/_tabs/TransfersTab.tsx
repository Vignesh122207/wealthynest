...
import {ArrowLeftRight, CalendarDays, Check, ChevronDown, Plus, Power} from "lucide-react";
...
function RuleFormModal({
  ...
  // above only evaluates once, so if useAccounts() resolves after mount "From Account" would
  // otherwise be stuck on no default forever. Sync once accounts arrive, but only for a fresh
  // rule and only if nothing's been picked yet.
  useEffect(() => {
    if (!isEdit && !fromAccountId && accounts[0]?.id) setFromAccountId(accounts[0].id);
      ...
      <div className="rounded-3xl overflow-hidden border border-border shadow-2xl animate-scale-in bg-card">
        ...
        <div className="p-5">
          ...
          <div className="space-y-4">
            ...
            <AccountPicker label="From Account" testId="recurring-transfer-from-account-picker"
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                type="button" disabled={saving || !valid} data-testid="recurring-transfer-form-submit"
...
function RuleCard({
  ...
  return (
    <div onClick={onEdit} data-testid="recurring-transfer-rule-card"
      className={cn(
      ...
      <button type="button" onClick={onEdit}
        ...
        Edit {rule.fromAccountName} to {rule.toAccountName} recurring transfer
      </button>
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <button onClick={() => setModal("create")}
            className="mt-4 flex items-center gap-2 h-9 px-4 rounded-xl text-sm font-medium bg-gradient-to-br from-indigo-600 to-indigo-500 shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/40 hover:-translate-y-0.5 text-white transition-all">
      ...
      <FloatingActionButton actions={[
        { icon: ArrowLeftRight, label: "Add Recurring Transfer", color: "indigo", onClick: () => setModal("create"), testId: "fab-add-recurring-transfer" },
      ]} />
    </>
  );
}
