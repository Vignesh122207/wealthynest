...
const mockedOpenPrintWindow = vi.mocked(openPrintWindow);
...
beforeEach(() => {
  vi.clearAllMocks();
  mockedExpensesApi.getExpenses.mockResolvedValue({ data: [], totalElements: 0, totalPages: 0, page: 0, size: 500 } as never);
  mockedApiClient.get.mockResolvedValue(transferResponse([]) as never);
});
...
describe("downloadAccountStatement", () => {
  it("defaults to all-time — no date bounds, scoped to this account's id", async () => {
    ...
    expect(mockedExpensesApi.getExpenses).toHaveBeenCalledWith(
      expect.objectContaining({ accountIds: ["acc-1"], size: 500, sortDir: "desc" })
    );
  ...
  it("filters transfers to only ones touching this account, and only within the requested year", async () => {
    ...
    const html = mockedOpenPrintWindow.mock.calls[0][1];
    expect(html).toContain("Transfers (1)");
  });
  ...
  it("does not filter transfers by year when 'all' is requested", async () => {
    ...
    const html = mockedOpenPrintWindow.mock.calls[0][1];
    expect(html).toContain("Transfers (2)");
  });

  it("escapes free-text expense descriptions before interpolating into the print window HTML", async () => {
    mockedExpensesApi.getExpenses.mockResolvedValue({
      data: [{ id: "e1", expenseDate: "2025-01-05", categoryName: "Groceries", description: "<script>alert(1)</script>", amount: 500, accountId: "acc-1" }],
      totalElements: 1, totalPages: 1, page: 0, size: 500,
    } as never);
  ...
  it("shows a toast instead of throwing when the expenses fetch fails", async () => {
    ...
    expect(mockedOpenPrintWindow).not.toHaveBeenCalled();
  });
});
